        ...
        if($total > 0){
            ...
            echo "<form id='formUpdateMarca' method='post' action='pew-update-marca.php' enctype='multipart/form-data'>";
                ...
                echo "<div class='label-full'>";
                    ...
                    echo "<input type='text' class='input-full' placeholder='Título da marca' name='titulo' id='tituloMarca' value='$titulo' maxlength='35'>";
                echo "</div>";
                echo "<div class='label-full'>";
                    echo "<h3 class='input-title'>Descrição (opcional, recomendado 156 caracteres)</h3>";
                    echo "<textarea class='input-full' placeholder='Descrição da marca' name='descricao' id='descricaoMarca'>$descricao</textarea>";
                echo "</div>";
                if(file_exists($dirImagens.$imagem) && $imagem != ""){
                    echo "<div class='label-full'>";
                            echo "<h3 class='input-title'>Imagem atual</h3>";
                            echo "<img src=".$dirImagens.$imagem." class='input-small'>";
                            echo "<br><br><br>";
                            echo "<h3 class='input-title'>Alterar imagem (300px : 300px)</h3>";
                            echo "<input type='file' class='input-half' name='imagem' accept='image/*'>";
                        echo "</div>";
                    echo "</div>";
                }else{
                    echo "<div class='label-full'>";
                        echo "<h3 class='input-title'>Imagem (300px : 300px)</h3>";
                        echo "<input type='file' class='input-full' name='imagem' accept='image/*'>";
                    echo "</div>";
                }
                echo "<div class='label-full'>";
                    echo "<h3 class='input-title'>Status</h3>";
                    echo "<select name='status' id='statusMarca' class='input-small'>";
                ...
                echo "<input type='button' class='btn-excluir botao-acao' pew-acao='deletar' pew-id-marca='$idMarca' value='Excluir'>";
